#!/usr/bin/env python
"""
Batch Trainer: Hybrid SIR + SVR Residual Corrector

Usage:
    python train_hybrid_batch.py --data_dir path/to/datasets --out_dir models/hybrid --lags 7

What it does:
    * Loads ALL .csv epidemic datasets in --data_dir (BIGBOY1 format or compatible).
    * Chooses target: Reported_Cases (preferred) else Infected.
    * Fits SIR baseline per dataset (to the target).
    * Computes residuals (target - SIR_pred).
    * Builds feature matrix using:
          - All numeric context cols (Beta_Effective, Season_Index, etc.) if present
          - Lagged target values (lag1..lagN)
          - SIR baseline prediction (SIR_Base)
    * Concats all datasets -> large training corpus
    * Chronological split within each dataset -> train/test (no leakage)
    * Scales features; trains global SVR on residuals
    * Evaluates SIR-only vs Hybrid (SIR+SVR) on held-out rows
    * Saves trained model, scaler, metrics, predictions

Notes:
    * Designed for BIGBOY1 output; gracefully handles missing optional columns.
    * Population inferred from S+I+R first row if available; else uses max of those present.
"""
...
# SIR Utilities
# ------------------------------------------------------------------
def _sir_forward(beta, gamma, N, I0, R0, T):
    """Discrete-time SIR forward simulation, returns arrays (S,I,R) length T."""
    S, I, R = N - I0 - R0, I0, R0
...
def _sir_curve_for_fit(t, beta, gamma, N, I0, R0):
    """Wrapper to return I(t) for curve_fit."""
    _, I, _ = _sir_forward(beta, gamma, N, I0, R0, len(t))
    return I


def fit_sir_to_series(y, N):
    """
    Fit SIR to observed series y (target). Returns beta, gamma, sir_I_pred.
    ...
    def f(t, beta, gamma):
        return _sir_curve_for_fit(t, beta, gamma, N, I0, R0)

    # bounds: avoid degenerate zeros
    popt, _ = curve_fit(
        f,
        t,
        y,
        p0=[0.2, 0.1],
        bounds=(0.0, [2.0, 1.0]),  # allow higher beta if needed
        maxfev=10_000,
    )
    beta_opt, gamma_opt = popt
    sir_pred = f(t, beta_opt, gamma_opt)
    return beta_opt, gamma_opt, sir_pred
...
def per_dataset_metrics(test_df, hybrid_pred):
    ...
    sir_base_all = test_df["SIR_Base"].values
...
def main():
    ...
    # Build hybrid predictions: SIR_Base + SVR residual
    ...
    overall_metrics = compute_metrics(y_true_test, sir_base_test, hybrid_pred)
